...
pub const Device = struct {
    ...
    pub fn find(identifier: u32) Error!Device {
        for (0..transport_count) |index| {
            const candidate = Device{ .base = transport_base + index * transport_stride };
            if (candidate.read(0) == magic_value and
                candidate.read(4) == modern_version and
...
pub fn Queue(comptime size: u16) type {
    ...
    return struct {
        ...
        pub fn pollCompletion(self: *Self) ?UsedElement {
            if (volatileRead(u16, &self.used.index) == self.last_used) return null;
            acquireBarrier();
            const slot = &self.used.ring[self.last_used % size];
            const element = UsedElement{
                .identifier = volatileRead(u32, &slot.identifier),
                .length = volatileRead(u32, &slot.length),
            };
            self.last_used +%= 1;
            return element;
        }
        ...
        pub fn wait(self: *Self, poll_limit: usize) Error!UsedElement {
            var polls: usize = 0;
            while (true) : (polls += 1) {
                if (self.pollCompletion()) |completion| return completion;
